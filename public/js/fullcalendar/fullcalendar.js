...
$.fn.fullCalendar = function(options) {
	...
	this.each(function(i, _element) { // loop each DOM element involved
		...
		if (typeof options === 'string') {
			...
			if (calendar && !$.isFunction(calendar[options])) {
				calendar.options[options] = args[0];
				//singleRes = calendar.options[options].apply(calendar, args);
			}
		}
...
var DragListener = Class.extend({
	...
	drag: function(ev) {
		...
		$(document).trigger('fullcalendarEventDragging',ev);
		if (this.isDragging) {
			cell = this.getCell(ev);
	...
	cellOver: function(cell,ev) {
		this.cell = cell;
		this.trigger('cellOver', cell, isCellsEqual(cell, this.origCell),ev);
	},
		...
		var dragListener = new DragListener(this.coordMap, {
			...
			listenStop: function(ev) {
				...
				if(selectionRange){
					...
					var _selectionRange = { start:selectionRange.start.stripZone(), end:selectionRange.end.stripZone() };
					$.each(allEvents,function(){
						
						if(this._id !== event._id){
							if((_selectionRange.start < this.end.stripZone() && this.start.stripZone() < _selectionRange.end)){
								if(!view.calendar.options.eventOverlapping(event)){
									dayClickCell = null;
									selectionRange = null;
									_this.destroySelection();
									disableCursor();
...
Grid.mixin({
	...
	segDragMousedown: function(seg, ev) {
		...
		var dragListener = new DragListener(view.coordMap, {
			...
			cellOut: function() { // called before mouse moves to a different cell OR moved out of all cells
				view.trigger('cellOut', el[0], event);
				dropLocation = null;
			...
			dragStop: function(ev) {
				...
				if(dropLocation){
					var dropLocationRange = { start:dropLocation.start.stripZone(), end:dropLocation.end.stripZone() };
					$.each(allEvents,function(){
						//console.log(this);
						//range.start >= eventStart && range.end <= eventEnd;
						if(this._id !== event._id){
							if((dropLocationRange.start < this.end.stripZone() && this.start.stripZone() < dropLocationRange.end)){
								if(!view.calendar.options.eventOverlapping(event)){
									dropLocation = null;
								}								 
							}						
				...
				//view.calendar.options.eventOverlapping(event);
				if(!dropLocation){
					view.trigger('beforeEventDragStop');
					mouseFollower.options.revertDuration=$(document).triggerHandler('beforeEventDragStop');					
				}
	...
	startExternalDrag: function(el, ev, ui) {
		...
		$(document).one('dragstop', function(ev, ui) {
			...
			//trueOrFalse = _this.view.calendar.options.eventOverlapping();
			var allEvents = _this.view.calendar.clientEvents();
				
			if(dropLocation){
				if(dropLocation.end === null){
					dropLocation.end = new moment(dropLocation.start.clone()).add(moment.duration(_this.view.calendar.options.defaultTimedEventDuration));
				}
				var dropLocationRange = { start:dropLocation.start.clone().stripZone(), end:dropLocation.end.clone().stripZone() };
				$.each(allEvents,function(){
					//console.log(this);
						//range.start >= eventStart && range.end <= eventEnd;
					if(this._id !== event._id){
						if((dropLocationRange.start < this.end.stripZone() && this.start.stripZone() < dropLocationRange.end)){
							if(!_this.view.calendar.options.eventOverlapping(event)){
								dropLocation = null;
							}								 
						}						
					}				
...
function EventManager(options) { // assumed to be a calendar
	...
	t.eventContainsRange = eventContainsRange;
